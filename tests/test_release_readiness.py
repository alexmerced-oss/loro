...
def test_bundled_release_contract_matches_runtime_and_freezes_cli() -> None:
    ...
    assert contract["release_line"] == "0.9"
    assert contract["stability"] == "release-candidate"
...
def test_release_readiness_cli_supports_warning_gate(monkeypatch, tmp_path) -> None:
    ...
    normal = runner.invoke(
        app, ["operations", "release-readiness", "--output", str(output)]
    )
    ...
    assert json.loads(output.read_text(encoding="utf-8"))["release_line"] == "0.9"
