...
| TM-07 | Local JSONL audit is modified, deleted, disabled, or lost during failure. | Actions cannot be reconstructed. | Local append-only-by-convention JSONL events. | Versioned schema, authenticated external sink, buffering/retry, visible failure, immutability or tamper evidence. | Batch 5 pending. |
...
normalized scopes are not an operating-system sandbox; local audit is not durable or
tamper-evident; sandbox, DLP, retention,
