...
| TM-15 | A malicious MCP server, negotiated downgrade, extension, skill instruction, script, reference, or asset attempts confused-deputy execution, credential theft, persistence, or policy bypass. | Remote code execution, data disclosure, or durable compromise. | MCP and Agent Skills are not currently implemented. | Version minimums, server/source allowlists, content trust labels, digest provenance, environment isolation, sandboxing, deny-by-default extensions, exact approvals, conformance and adversarial fixtures. | [MCP and Agent Skills roadmap](mcp-skills-roadmap.md); implementation and review pending. |
