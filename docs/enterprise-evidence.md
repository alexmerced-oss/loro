...
| E3-03 | Audit evidence is immutable or tamper-evident | Operations/security (TBD) | Partial | JSONL and buffer writes are process-locked; a backward-compatible SHA-256 chain and `loro audit verify` detect retained-file mutation, with optional external final-hash anchors. Immutable collector, independent anchors, retention, and outage proof remain external. |
| E3-04 | Metrics/traces cover latency, denial, approval, cost, memory, and audit | Operations/runtime (TBD) | Partial | Runtime audit uses a task trace id and records model/tool/task latency, denials, approvals, stop reason, and content-free usage. Production exporter, dashboards, SLOs, and alerts remain external. |
...
| ID | Exit item | Owner | Status | Evidence and remaining proof |
| --- | --- | --- | --- | --- |
| E4-01 | Postgres and Polaris/Iceberg integration runs automatically in protected CI | Release/data (TBD) | Partial | Weekly/manual CI runs ephemeral Postgres and pinned Polaris `1.5.0` quickstart smoke. Credentialed Polaris CLI and full governed Iceberg transactions require a protected external runner/environment. |
| E4-02 | End-to-end reference deployment tests pass | Release/security (TBD) | Partial | A hermetic identity-to-approval-to-tool-to-session-to-hash-chain E2E test exists in `tests/test_runtime.py`; production-like Postgres, gateway, DLP, and external-audit report remains external. |
