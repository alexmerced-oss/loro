...
- **Existing:** evidence is present, though it may prove only pre-GA behavior.
...
| E3-05 | Gateway resilience and policy-safe fallback are proven | Runtime (TBD) | Partial | Provider adapters add bounded timeout/network/429/5xx retries, CA/proxy support, managed host allowlists, request IDs, usage budgets, and explicit-only graph tier routing. Sanitized contracts and protected live jobs exist; enterprise residency, production rate/load, and route approval remain external. |
...
| E4-02 | End-to-end reference deployment tests pass | Release/security (TBD) | Partial | Hermetic identity-to-approval-to-tool-to-session-to-hash-chain coverage, ephemeral Postgres lifecycle/recovery, audit outage drills, and a content-free installed-environment readiness report exist; production gateway, corporate DLP, immutable audit, and protected Polaris reports remain external. |
| E4-03 | Provider contracts use sanitized fixtures and controlled live smoke | Runtime/release (TBD) | Partial | Wheel-bundled sanitized contracts cover four advertised cloud protocol families; the machine-readable matrix, offline CLI validator, hostile tests, and protected explicit-route live workflow exist. Six configured routes passed local content-free streaming smoke on August 11, 2026; a green protected artifact for every organization-approved account/route remains external release evidence. |
...
| E4-06 | Release has SBOM, provenance, signature, and verification | Release (TBD) | Partial | Security CI emits CycloneDX; tag/manual release CI bundles a clean-environment SBOM, frozen release contract, readiness report, matrices, artifact-bound manifest, checksums, and GitHub/Sigstore provenance. Consumer verification instructions exist; protected-tag/trusted-publishing administration and release-owner verification remain external. |
| E4-07 | Upgrade, rollback, migration, support, and disclosure policies work | Release/operations (TBD) | Partial | Root configuration schema `1.0`, legacy migration, future-version refusal, frozen release contract, deprecation/support/vulnerability policies, runbooks, and tests exist. Full managed deployment upgrade/rollback, storage migration, support, and disclosure exercises remain external. |
| E4-08 | Advertised MCP revisions and Agent Skills pass conformance/interoperability | Release/runtime (TBD) | Partial | Dual-era SDK and runner workflows, downgrade/capability-confusion fixtures, Agent Skills validation, and a frozen Claude/Pi subset exist. A green protected workflow artifact remains required for each release commit. |
| E4-09 | AGS 1.0 graphs and run records pass pinned conformance | Release/runtime (TBD) | Partial | Vendored schemas/reference validator, upstream fixtures, strict examples, pinned workflows, and failure injection cover approval, retry, fallback, compensation declaration, resume, and budgets. Protected release-commit evidence and an organization-approved live-provider run remain gates. See [Agentic Graphs](agentic-graphs.md). |
| E4-10 | Remote channel work is authenticated, tenant-bound, replay-safe, and auditable | Runtime/security (TBD) | Partial | Six adapters have signed fixtures, explicit event-type rejection, identity/channel/workspace policy, durable replay suppression, credential-rotation rejection, overload bounds, audit correlation, and OS-vaulted credentials. TLS ingress, platform app governance, outage, and production hostile-event evidence remain external. See [Channel Gateways](channel-gateways.md). |
...
| E5-02 | Admin/operator/user/privacy/incident docs published | Product/operations (TBD) | Existing | The [enterprise beta guide](enterprise-beta.md), [operator runbook](operator-runbook.md), [security/privacy guide](security-privacy-review.md), [release-candidate operations](release-candidate.md), assurance/consumer guides, support policy, integration docs, and user guides cover the repository-owned surface. Deployment contacts and organization procedures remain external. |
...
| E5-08 | Supported and unsupported use cases are published | Product (TBD) | Existing | The support matrix, frozen [release contract](release-contract.json), [enterprise beta guide](enterprise-beta.md), [reference deployment](reference-deployment.md), and [support policy](support-policy.md) publish the candidate boundary; GA promotion remains gated. |
